                ...
                <div className="pt-6 border-t border-white/10">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {movie.platforms.map((platform: any) => (
                      <div
                        key={platform.id}
                        className={`p-4 rounded-lg border transition-all ${platform.available ? "bg-zinc-900 border-zinc-700 hover:border-red-600" : "bg-zinc-950 border-zinc-800 opacity-50"}`}
                      >
                        <div className="flex items-center gap-3">
                          <span className="text-3xl">{platform.logo}</span>
                          <div>
                            <p className="text-white font-medium text-sm">{platform.name}</p>
                            <p className={`text-xs ${platform.available ? "text-green-500" : "text-red-500"}`}>
